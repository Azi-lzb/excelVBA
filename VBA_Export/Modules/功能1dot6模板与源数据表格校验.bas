...
Private Function 表格样式校验(ByVal templateWb As Workbook, ByVal sourceWb As Workbook, ByVal 执行行区域 As Boolean, ByVal 执行列区域 As Boolean, ByVal 强制按模板 As Boolean) As String
    ...
    If 强制按模板 Then
        ...
        ' 用模板「模板」表对所有源 sheet 的各自矩形区域比对，不合并同行/列区域
                ...
                If UBound(arr) >= 3 Then
                    ...
                    For r = minR To maxR
                        ...
                        For c = minC To maxC
                            ...
                            cellKey = srcWs.Name & "|" & CStr(r) & "|" & CStr(c)

                            If Not checkedCells.Exists(cellKey) Then
    ...
    Else
        ...
        ' 每个源 sheet 只和对应模板 sheet 的各自矩形区域比对，不再把同编号行/列区域合并成大矩形
