Attribute VB_Name = "功能3dot14按批注删除重复行"
Option Explicit
...
Public Sub ExecuteDedupDeleteByComment()
    Application.Run "功能3dot14_按批注删除重复行"
End Sub

Public Sub 功能3dot14_按批注删除重复行()
    ...
    If Len(pickedPath) = 0 Then
        MsgBox "已取消选择工作簿。", vbInformation, "按批注删除重复行"
        Exit Sub
    End If

    resolvedPath = ResolveWorkbookPath(pickedPath)
    If IsDirectoryPath(resolvedPath) Then
        MsgBox "当前选择是文件夹，请选择Excel/CSV文件。", vbExclamation, "按批注删除重复行"
        Exit Sub
    End If
    If Not IsSupportedWorkbookFilePath(resolvedPath) Then
        MsgBox "仅支持 xls/xlsx/xlsm/xlsb/csv 文件。", vbExclamation, "按批注删除重复行"
        Exit Sub
    End If
    If Not FileExists(resolvedPath) Then
        MsgBox "文件不存在，请重新选择。", vbExclamation, "按批注删除重复行"
        Exit Sub
    End If
    ...
    If wb Is Nothing Then
        ...
        On Error GoTo 0
        If wb Is Nothing Then
            MsgBox "打开工作簿失败：" & CStr(errNo) & " " & errDesc, vbCritical, "按批注删除重复行"
            Exit Sub
        End If
        openedByCode = True
    End If
    ...
    For Each ws In wb.Worksheets
        If CellCommentContains(ws.cells(1, 1), MARK_SHEET_TEXT) Then
            ...
            deletedRowCount = deletedRowCount + DeleteDuplicateRowsByComment(ws)
        End If
    Next ws
    ...
    MsgBox "按批注删除重复完成。" & vbCrLf & _
           "命中Sheet数：" & hitSheetCount & vbCrLf & _
           "删除重复行数：" & deletedRowCount, vbInformation, "按批注删除重复行"
    Exit Sub
...
FailHandler:
    ...
    If openedByCode Then
        ...
        On Error GoTo 0
    End If
    MsgBox "执行失败：" & CStr(errNo) & " " & errDesc, vbCritical, "按批注删除重复行"
End Sub
...
Private Function DeleteDuplicateRowsByComment(ByVal ws As Worksheet) As Long
    ...
    DeleteDuplicateRowsByComment = DeleteRowsByCollectionBatch(ws, rowsToDelete)
End Function
